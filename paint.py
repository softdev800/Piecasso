...
MODES = [
    'selectpoly', 'selectrect', 'eraser', 'fill', 'dropper', 'stamp', 'pen', 'brush',
    'spray', 'text', 'line', 'polyline', 'rect', 'polygon', 'ellipse', 'roundrect'
]
...
class Canvas(QLabel):
    ...
    def initialize(self):
        ...
        if self.secondary_color and self.eraser_color:
    ...
    def reset(self):
        self.setPixmap(QPixmap(*CANVAS_DIMENSIONS))  # Create the pixmap for display.
        self.pixmap().fill(self.background_color)  # Clear the canvas.
    ...
    def set_mode(self, mode):
        self.timer_cleanup()  # Clean up active timer animations.
    ...
    def timer_cleanup(self):
        if self.timer_event:  # Stop the timer, then trigger cleanup.
    ...
    def selectpoly_copy(self):
        ...
        p.end()
        pixmap.setMask(bitmap)  # Set our created mask on the image.
    ...
    def eraser_mouseMoveEvent(self, e):
        if self.last_pos:
            ...
            p.setPen(QPen(
                self.eraser_color, 30, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin
            ))
    ...
    def pen_mouseMoveEvent(self, e):
        if self.last_pos:
            p = QPainter(self.pixmap())
            p.setPen(QPen(
                self.active_color, self.config['size'], Qt.SolidLine, Qt.SquareCap, Qt.RoundJoin
            ))
            p.drawLine(self.last_pos, e.pos())
            self.last_pos = e.pos()
            self.update()
    ...
    def brush_mouseMoveEvent(self, e):
        if self.last_pos:
            p = QPainter(self.pixmap())
            p.setPen(QPen(
                self.active_color, self.config['size'] * BRUSH_MULT, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin
            ))
            p.drawLine(self.last_pos, e.pos())
            self.last_pos = e.pos()
            self.update()
    ...
    def text_mousePressEvent(self, e):
        if e.button() == Qt.LeftButton and self.current_pos is None:
            ...
            self.timer_cleanup()  # Draw the text to the image
            p = QPainter(self.pixmap())
            p.setRenderHints(QPainter.Antialiasing)
            font = build_font(self.config)
            p.setFont(font)
            pen = QPen(
                self.primary_color, 1, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin
            )
            p.setPen(pen)
    ...
    def generic_shape_mouseReleaseEvent(self, e):
        if self.last_pos:
            self.timer_cleanup()  # Clear up indicator.
            p = QPainter(self.pixmap())
            p.setPen(QPen(
                self.primary_color, self.config['size'], Qt.SolidLine, Qt.SquareCap, Qt.MiterJoin
            ))
    ...
    def line_mouseReleaseEvent(self, e):
        if self.last_pos:
            ...
            p = QPainter(self.pixmap())
            p.setPen(QPen(
                self.primary_color, self.config['size'], Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin
            ))
    ...
    def generic_poly_mousePressEvent(self, e):
        if e.button() == Qt.LeftButton:
            ...
            self.timer_cleanup()  # Clean up, we're not drawing
            self.reset_mode()
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle("Piecasso")
        self.setWindowIcon(QIcon(':/icons/piecasso.ico'))
        self.show()
